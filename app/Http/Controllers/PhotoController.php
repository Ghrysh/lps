...
class PhotoController extends Controller
{
    ...
    public function photobooth()
    {
        ...
        $bajuAdat = PhotoboothDataset::all()->sortBy('title');

        // jika ingin mengubah ke array biasa (opsional)
        $bajuAdat = PhotoboothDataset::all()->sortBy('title')->map(function ($item) {
            return [
                'id' => $item->id,
                'name' => $item->title,
                'image' => $item->image_url,
            ];
        })->values(); // penting: reset key supaya jadi array numerik
