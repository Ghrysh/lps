...
@section('title', 'Dataset Photobooth')

@section('content')
    <div class="mb-8 flex justify-between items-center">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Photobooth Dataset</h2>
            <p class="text-sm text-slate-500">Dataset untuk AR & AI Recognition</p>
        </div>
        <button onclick="document.getElementById('modalUpload').classList.remove('hidden')"
            class="bg-orange-600 text-white px-4 py-2 rounded-lg hover:bg-orange-700 transition">
            <i class="fas fa-plus mr-2"></i> Upload Dataset
        </button>
    </div>
    ...
    <div id="modalUpload" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center">
        <div class="bg-white p-6 rounded-2xl w-full max-w-md">
            <h3 class="font-bold text-lg mb-4">Upload Dataset Photobooth</h3>
            <form action="{{ route('admin.photobooth.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="space-y-4">
                    <input type="text" name="title" placeholder="Judul Dataset" class="w-full p-2 border rounded-lg"
                        required>
