...
<body class="min-h-screen flex flex-col relative bg-slate-50">
                        ...
                        <div class="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                            @foreach ($bajuAdat as $baju)
                                <button onclick="selectBaju('{{ $baju['id'] }}')"
                                    ...
                                    data-id="{{ $baju['id'] }}">
                                    {{-- Gunakan accessor image_url --}}
                                    <img src="{{ $baju['image'] ?? asset('assets/baju/default.png') }}"
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="text-[9px] text-white font-bold block truncate">{{ $baju['name'] }}</span>
                                    </div>
                                </button>
                            @endforeach
                        </div>
    ...
    {{-- 
        HIDDEN AREA UNTUK PRINT 
        Penting: Class 'hidden' akan ditimpa oleh CSS @media print
    --}}
    <div id="print-area" class="hidden">
        <img id="print-image-src" alt="Hasil Foto">
    </div>
    <script>
        const assetsData = @json($bajuAdat);
    ...
    <script type="module">
        ...
        function loadAssets() {
            ...
            const basePath = "{{ asset('assets/baju') }}"; // tetap dipakai
        ...
        window.printPolaroid = () => {
            ...
            logo.src = "{{ asset('assets/logo-lps.png') }}";
        ...
        function logBajuClick(datasetId) {
            fetch("{{ route('photobooth.baju.click') }}", {
                method: "POST",
                headers: {
