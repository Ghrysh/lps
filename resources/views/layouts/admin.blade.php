...
<body class="min-h-screen text-slate-600">
    ...
    <div id="sidebarOverlay" class="fixed inset-0 bg-slate-900/50 z-40 hidden lg:hidden backdrop-blur-sm"
    ...
    <aside id="sidebar"
        ...
        <nav class="flex-grow py-6 overflow-y-auto px-4 space-y-1">
            ...
            <a href="{{ route('admin.dashboard') }}"
                ...
                <i
                    class="{{ request()->routeIs('admin.dashboard') ? 'text-emerald-600' : 'text-slate-400 group-hover:text-slate-600' }} fas fa-home w-5 transition-colors mr-3"></i>
                <span>Dashboard</span>
            </a>
            ...
            {{-- === DROPDOWN DATASET (UPDATED) === --}}
            <div>
                <button onclick="toggleDropdown('masterDropdownDataset')" id="btn-masterDropdownDataset"
                    class="{{ request()->is('admin/gallery*') || request()->is('admin/photobooth-dataset') || request()->routeIs('slider.*') ? 'text-slate-800 font-medium bg-slate-50' : 'text-slate-500' }} w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-slate-50 rounded-lg transition group">
                    <div class="flex items-center">
                        <i class="fas fa-database w-5 mr-3 text-slate-400 group-hover:text-slate-600"></i>
                        <span>Dataset</span>
                    </div>
                ...
                <div id="masterDropdownDataset"
                    class="dropdown-container pl-4 space-y-1 mt-1 {{ request()->routeIs('admin.gallery.*') || request()->routeIs('admin.photobooth.*') || request()->routeIs('slider.*') ? 'open' : '' }}">
                    ...
                    {{-- 2. Dataset Photobooth --}}
                    <a href="{{ route('admin.photobooth.index') }}"
                        class="{{ request()->routeIs('admin.photobooth.index') ? 'text-emerald-600 font-medium bg-emerald-50' : 'text-slate-500 hover:text-slate-800' }} block px-4 py-2 text-sm rounded-lg transition border-l-2 {{ request()->routeIs('admin.photobooth.index*') ? 'border-emerald-500' : 'border-transparent hover:border-slate-300' }}">
                        <span>Dataset Photobooth</span>
                    </a>

                    {{-- 3. Dataset Slider (NEW) --}}
                    <a href="{{ route('admin.slider.index') }}"
                        class="{{ request()->routeIs('admin.slider.*') ? 'text-emerald-600 font-medium bg-emerald-50' : 'text-slate-500 hover:text-slate-800' }} block px-4 py-2 text-sm rounded-lg transition border-l-2 {{ request()->routeIs('admin.slider.*') ? 'border-emerald-500' : 'border-transparent hover:border-slate-300' }}">
                        <span>Dataset Slider</span>
                    </a>
            ...
            <div>
                <button onclick="toggleDropdown('masterDropdown')" id="btn-masterDropdown"
                    class="{{ request()->is('admin/tools*') || request()->is('admin/scanner*') || request()->routeIs('tools.quiz_manager') ? 'text-slate-800 font-medium bg-slate-50' : 'text-slate-500' }} w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-slate-50 rounded-lg transition group">
                    <div class="flex items-center">
                ...
                <div id="masterDropdown"
                    class="dropdown-container pl-4 space-y-1 mt-1 {{ request()->routeIs('admin.scanner') || request()->routeIs('admin.points.scan') || request()->routeIs('tools.quiz_manager') ? 'open' : '' }}">
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50/30">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 overflow-hidden">
                    <div
                        class="w-9 h-9 flex-shrink-0 rounded-full bg-emerald-100 border border-emerald-200 flex items-center justify-center text-sm font-bold text-emerald-700">
                        {{ substr(auth()->user()->name ?? 'A', 0, 1) }}
                    </div>
                    <div class="overflow-hidden">
                        <p class="text-sm font-semibold text-slate-700 truncate leading-none mb-0.5">
    ...
    <div id="mainContent" class="sidebar-transition flex-col min-h-screen lg:ml-64 flex">
        ...
        <header id="mobileHeader"
            ...
            <div class="flex items-center">
                <button onclick="toggleSidebar()" class="text-emerald-600 p-2 focus:outline-none">
        ...
        <header id="desktopHeader"
            ...
            <div class="flex items-center">
                <button id="desktopHamburger" onclick="toggleSidebar()"
                    class="text-emerald-600 p-2 focus:outline-none mr-4 hidden">
                    <i class="fas fa-bars text-xl"></i>
                </button>
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div
                        class="w-9 h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold uppercase">
    ...
    <script>
        ...
        function toggleDropdown(id) {
            ...
            if (dropdown.classList.contains('open')) {
                ...
                if (!btn.classList.contains('bg-slate-50')) {
