...
<body class="flex flex-col h-screen overflow-hidden bg-slate-50">
    ...
    {{-- 3. BOTTOM NAVIGATION --}}
    <nav
        class="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 h-[75px] z-50 rounded-t-2xl shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
        ...
        <div class="grid grid-cols-5 h-full max-w-lg mx-auto relative">
            ...
            <a href="{{ route('visitor.index') }}"
                class="flex flex-col items-center justify-center gap-1 {{ request()->routeIs('visitor.index') ? 'active-nav' : 'inactive-nav' }}">
            ...
            <a href="{{ route('visitor.map') }}"
                class="flex flex-col items-center justify-center gap-1 {{ request()->routeIs('visitor.map') ? 'active-nav' : 'inactive-nav' }}">
            ...
            <a href="{{ route('visitor.scan.ar') }}"
                class="flex flex-col items-center justify-center gap-1 {{ request()->routeIs('visitor.scan.ar') ? 'active-nav' : 'inactive-nav' }}">
                <i class="fas fa-cube text-xl mb-0.5"></i>
                <span class="text-[10px]">AR Scan</span>
            </a>

            {{-- 5. KELUAR (Kanan Paling Ujung) --}}
            <form action="{{ route('visitor.logout') }}" method="POST"
                class="flex flex-col items-center justify-center">
                @csrf
                <button type="submit"
                    class="flex flex-col items-center justify-center gap-1 text-slate-400 hover:text-red-500 transition w-full h-full">
