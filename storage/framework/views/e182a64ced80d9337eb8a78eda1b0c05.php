    ...
    <style>
        ...
        @media print {
            ...
            #print-area {
                ...
                /* Paksa tampil menimpa class 'hidden' */
                position: fixed;
                top: 0;
                left: 0;
                width: 100%;
            ...
            #print-image-src {
                max-width: 90%;
                /* Beri margin agar tidak terpotong printer */
                max-height: 90%;
                width: auto;
                height: auto;
                object-fit: contain;
...
<body class="min-h-screen flex flex-col relative bg-slate-50">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div class="lg:col-span-1 space-y-4">
                    ...
                    <div class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100">
                        <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Mode
                            Pakaian</label>
                        <div class="grid grid-cols-2 gap-2">
                            <button onclick="setGenderMode('pria')"
                                class="gender-btn py-2 px-3 rounded-xl text-xs font-bold border hover:bg-orange-50 transition active-gender"
                                data-mode="pria">
                                <i class="fas fa-male mr-1"></i> Pria
                            </button>
                            <button onclick="setGenderMode('wanita')"
                                class="gender-btn py-2 px-3 rounded-xl text-xs font-bold border hover:bg-orange-50 transition"
                                data-mode="wanita">
                    ...
                    <div class="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex-grow">
                        <label class="block text-xs font-bold text-slate-700 mb-2 uppercase tracking-wider">Koleksi
                            Baju</label>
                        <div class="grid grid-cols-2 gap-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                            <?php $__currentLoopData = $bajuAdat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $baju): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <button onclick="selectBaju('<?php echo e($baju['id']); ?>')"
                                    ...
                                    <img src="<?php echo e($baju['image'] ?? asset('assets/baju/default.png')); ?>"
                                    ...
                                    <div
                                        ...
                                        <span
                                            class="text-[9px] text-white font-bold block truncate"><?php echo e($baju['name']); ?></span>
                                    </div>
                                </button>
                ...
                <div class="lg:col-span-3">
                    <div class="relative bg-black rounded-3xl overflow-hidden shadow-2xl aspect-video group">
                        ...
                        <div id="loading-cam"
                            class="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/90 text-white">
                        ...
                        <div
                            class="absolute bottom-6 left-0 right-0 flex justify-center z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <button onclick="takeSnapshot()"
                                class="bg-white/20 backdrop-blur-md border-4 border-white h-16 w-16 rounded-full flex items-center justify-center hover:bg-orange-600 hover:border-orange-600 transition-all shadow-lg transform hover:scale-110">
    ...
    <div id="result-modal"
        ...
        <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden transform transition-all scale-95 opacity-0"
            id="modal-content">
            ...
            <div class="p-6 bg-slate-50 text-center">
                ...
                <div class="flex gap-3 justify-center">
                    <button onclick="printPolaroid()"
                        class="bg-slate-800 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-slate-900 shadow-lg flex items-center gap-2">
                        <i class="fas fa-print"></i> Cetak Foto
                    </button>
                    <button onclick="closeModal()"
                        class="bg-white text-slate-700 border border-slate-300 px-6 py-3 rounded-xl font-bold text-sm hover:bg-slate-50 flex items-center gap-2">
    ...
    <script>
        const assetsData = <?php echo json_encode($bajuAdat, 15, 512) ?>;
    ...
    <script type="module">
        ...
        function loadAssets() {
            ...
            const basePath = "<?php echo e(asset('assets/baju')); ?>"; // tetap dipakai
            const baju = assetsData.find(b => b.id === state.bajuId); // cari data baju sesuai id

            if (!baju) return;

            genders.forEach(g => {
                const key = `${baju.id}_${g}`;
                if (!state.assets[key]) {
                    const img = new Image();

                    // jika baju.image sudah full URL, pakai langsung
                    if (baju.image.startsWith('http')) {
                        img.src = baju.image;
                    } else {
                        img.src = `${basePath}/${baju.image}`;
                    }
        ...
        function logBajuClick(datasetId) {
            fetch("<?php echo e(route('photobooth.baju.click')); ?>", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    photobooth_dataset_id: datasetId,
                    gender_mode: state.genderMode,
...
<?php /**PATH /var/www/html/resources/views/admin/tools/photobooth.blade.php ENDPATH**/ ?>